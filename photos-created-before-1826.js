document.querySelector('#runBtn').addEventListener('click', e => {
  e.preventDefault();
  const url = endpoint + 'ksamsok/api?x-api=test&method=search&hitsPerPage=500&fields=url,create_fromTime,create_toTime&query=itemType=foto%20AND%20create_fromTime<1826%20AND%20serviceOrganization=' + select.value + '&recordSchema=xml';
